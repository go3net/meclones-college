...
export default async function TeacherThreadPage({ params }: Props) {
  ...
  if (!thread) notFound();
  if (thread.teacher.userId !== user.id) redirect("/portal/teacher/messages");
  ...
  return (
    <PortalShell role="teacher">
      <div className="mb-4 flex items-center gap-3">
        <Link href="/portal/teacher/messages" className="text-slate-500 hover:text-brand-700">
      ...
      <ThreadView
        ...
        counterpartName={thread.parent.user.name}
        studentLabel={thread.student?.user.name ?? null}
        currentUserId={user.id}
        peerLastReadAt={thread.parentLastReadAt}
